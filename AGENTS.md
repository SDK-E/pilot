...
- The chat interface may show only capabilities backed by the protected runtime contract. Mark pending agents such as Research unavailable; do not add attachment, streaming, reasoning, tool, browser, scratchpad, or approval controls until their events, persistence and authorization behavior exist.
- Execution and activity records are Pilot-owned and organization-scoped. Record lifecycle events from verified server work only; never persist reasoning, tool secrets, or browser-supplied activity as an audit event.
