...
- Conversations are Pilot-owned, organization-scoped sessions. Their message records and protected runtime client must remain server-only. Do not enable message submission for an environment until its Neon-backed Pilot runtime and authenticated transport are configured and verified; do not expose tools.
