...
- Tools are `web-search`, `scratchpad`, `ask-user`, `plan`, `code-sandbox`, and 8 connector tools (`connector-github`, `connector-google-drive`, `connector-gmail`, `connector-slack`, `connector-notion`, `connector-linear`, `connector-vercel`, `connector-monday`), listed in `src/agents/agent-tools.ts`. An enabled tool that is allowed for the agent's kind just runs — there is no approval or suspension step, matching Claude Code/Codex. Web search and the code sandbox are also gated by the organization's `webSearchEnabled`/`codeSandboxEnabled` preferences (`src/organizations/organization-preference-repository.ts`, editable from Settings by owners/admins — not an environment variable, so toggling one needs no deploy); pilot-ai still enforces its own `PILOT_ENABLE_WEB_SEARCH`/`PILOT_ENABLE_CODE_SANDBOX` env flags as a platform-level circuit breaker independent of any org's setting. The sandbox runs each call in a fresh Vercel Sandbox with no access to Pilot's own systems, secrets, or data. A connector tool is gated by whether an active OAuth connection (personal or organization-wide) exists for its provider (`src/connectors/`, [ADR-0021](docs/decisions/0021-connectors.md)) — connection existence is itself the enablement signal, no separate preference boolean — plus pilot-ai's own `PILOT_ENABLE_CONNECTORS` circuit breaker. All 8 connector tools are read-only; do not add a write/mutating connector action without an equivalent durable approval workflow.
