...
- Projects are Pilot-owned, creator-scoped collections. Project membership and instructions are configuration only until a server-verified runtime contract makes them effective. Never use a project to bypass private-chat ownership, inject project content into an agent request, or enable project files, knowledge, or cross-chat memory without explicit storage, retrieval, deletion, and authorization behavior.
