...
5. Add Neon-backed Mastra message history for a selected conversation and prove real two-turn memory after a fresh process restart.
...
`pilot-ai` has unreviewed parallel browser/research work but has no approved runtime contract or connection to Pilot. It must not be deployed or called until Pilot can provide tenant-scoped authorization, a model gateway, and Neon-backed durable storage. `pilot-integrations` and `pilot-ui` remain package stubs. Pilot exposes empty conversation sessions but has no messages, tasks, runtime, approvals, memory or integration capabilities yet.
