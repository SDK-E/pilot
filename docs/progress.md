...
5. Configure `PILOT_MASTRA_DATABASE_URL` in each matching Pilot runtime environment, deploy its protected endpoint, and configure Pilot as its Vercel Trusted Source.
6. Set `PILOT_AI_RUNTIME_URL` for the matching Pilot environments and prove one real two-turn conversation, including a fresh runtime process reading the first message.
...
`pilot-ai` contains separate `src/pilot` and `src/pilot-research` entrypoints. Pilot Research remains local-development work with no tenant-scoped service adapter or approved deployment path. The Pilot entrypoint has no tools. `pilot-integrations` and `pilot-ui` remain package stubs. Pilot has no tasks, approvals, memory controls or integration capabilities yet.
