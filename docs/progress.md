...
only sanitized execution and tool summaries from the current response; it is
collapsed by default. On narrow screens, the full chat-details rail is also collapsed by default. The
...
## Migration journal reconciliation (2026-09-14)

`drizzle/meta/_journal.json` only listed entries through `0021`, so `0022_user_locale.sql` through `0026_conversation_panel_layout.sql` — plus a long-orphaned `0020_lifecycle_operations.sql` from the original platform slice that was never journaled at all — were invisible to `drizzle-kit migrate`. Because `vercel.json`'s production build already runs `drizzle-kit migrate` when `VERCEL_ENV=production`, this was a live gap: a production deploy would have silently skipped all six files and left the production schema behind `src/db/schema.ts`, producing column/table-not-found errors at runtime. The dev Neon database had these changes only because they were applied by hand, bypassing the tracked journal.

Fixed by appending journal entries (`idx` 22–27, monotonically increasing `when`) for the five 0022–0026 files and for the orphaned `0020_lifecycle_operations.sql` (kept at its original filename, journaled last since it was never assigned a slot). No `.sql` file content changed. Verified against a disposable local Postgres container (not the shared dev Neon database): applied `0000`–`0021` to build a clean baseline, then applied the corrected journal on top — all 28 migrations ran cleanly via `drizzle-kit migrate`, and every expected 0022–0026/0020 table and column landed correctly. Migration snapshot JSON files (`drizzle/meta/NNNN_snapshot.json`) are unaffected by this fix and remain a separate, pre-existing gap (missing for `0015` and `0021`); they are used only by `drizzle-kit generate`'s diffing, not by `migrate`, so they do not block applying migrations.

The dev Neon database's own `drizzle."__drizzle_migrations"` tracking table still only has rows through `0021` (the hand-applied 0022–0026/0020 changes were never recorded there). Running `pnpm db:migrate` against it now would attempt to re-run those six files; since all of them use `IF NOT EXISTS`/`OR REPLACE` this should no-op safely, but it was not attempted against the shared dev database as part of this fix and should be verified deliberately before relying on it.

## Composer: attachments, connectors, skills (2026-09-16)

The chat composer and edit-message flow gained three Claude.ai/ChatGPT-style capabilities, plus a new **skills** concept (`src/skills/`): an organization-defined bundle of extra instructions and/or tool grants (`skills` table, migration `0040_mighty_madrox.sql`), assignable per agent (`workers.enabledSkillIds`) from a new top-level `/skills` section (list/new/edit, mirroring `/agents`) and from the agent form, or selected per message from the composer's new skills picker. A skill contributes only what the request to pilot-ai already understood — extra `instructions` text and additional `allowedToolIds` — so pilot-ai itself needs no changes to support it.

The composer's "+" menu gained the already-built-but-unwired screenshot action and a broadened attachment allowlist (`.pptx`, `.gif`, `.svg`, JSON/XML/HTML/zip, shared between the composer and `src/files/private-file-policy.ts`), plus page-wide drag-and-drop (`PromptInput`'s existing `globalDrop` prop, now passed). A separate new toggle (`ConnectorToggleMenu`) lists the agent's already-connected connector tools and defaults every one on for each message — consistent with the existing "connection existence is itself the enablement signal" rule — letting the user turn specific ones off per message; it can only narrow, never widen, what `tool-authorization.ts`'s `allowedToolIds()` already grants. Both the connector toggle and the skill picker's selections are persisted on the sent `conversation_messages` row (`skill_ids`, `connector_tool_ids` columns) so a past turn keeps a record of what was active on it, and an edit-and-resend reconstructs the same selection.

Editing a past user message (`UserMessageEditor`) now also edits its attachments, not just its text. This needed `conversation_attachments` to gain a nullable `message_id` (previously conversation-scoped only, meaning every attachment was visible to every turn) — `forgetMessagesFrom` now detaches any attachment the edit keeps before truncating the old message (so it survives), deletes the blob of anything genuinely removed, and the new message re-parents the kept-plus-newly-uploaded set once it exists (`attachConversationAttachmentsToMessage`).

**Known residual risk, not fixed in this slice**: `src/ai/runtime-contract.ts`'s `allowedToolIds` cap was raised from 5 to 20 to keep this feature from throwing at the Zod boundary (the 5 base tools alone already reached the old cap with zero connectors connected — a latent issue independent of this feature). This file is Pilot's own hand-maintained mirror of pilot-ai's contract ([ADR-0013](decisions/0013-shared-contracts-package.md) is not yet implemented); pilot-ai's own copy of this cap has not been verified or raised as part of this change and needs separate confirmation before relying on more than 5 tools reaching the runtime in production.

Verified locally: `pnpm check` (lint, typecheck, format, knip — all clean), `pnpm test:server` (73/73, including new `tool-authorization`/`agent-skill-grants` cases), `pnpm test:db` (the edit/regenerate/continue suite passed unchanged; 7 pre-existing, unrelated `connector-repository` integration failures traced to this environment's `CONNECTOR_TOKEN_ENCRYPTION_KEY` being the wrong byte length, not to this change). Authenticated browser verification of the new composer UI was not performed — this session's dev server port was held by another session.

## Platform admin panel and model gateways (2026-09-17)

Pilot gained a platform-wide admin layer, separate from any organization's own owner/admin roles: `platform_admins` (`src/platform/`), keyed by WorkOS user id with a `superadmin`/`admin` role, bootstrapped once via `pnpm admin:bootstrap <workos-user-id> <email>` (no in-app way to create the first one, by design — the panel itself requires already being an admin). `/admin` (outside the `(workspace)` route group, gated by `requirePlatformAdmin()`, added to `src/proxy.ts`'s matcher) has two sections: Admins (add/remove other admins by email lookup through WorkOS, superadmin-only; a safeguard refuses removing the last superadmin) and Model gateways.

Model gateways (`src/model-gateways/`, `model_gateways` table) replace the previous hardcoded assumption that every model call goes through Kilo Gateway via `pilot-ai`'s own `KILO_API_KEY` env var. A gateway is just a name, a base URL, an encrypted API key (AES-256-GCM, its own `MODEL_GATEWAY_ENCRYPTION_KEY`, separate from the connector-token key), and a curated list of allowed model ids — Kilo Gateway, Vercel AI Gateway, and any other provider are all the same shape, since all three speak the OpenAI-compatible chat-completions wire format; the admin form offers one-click base-URL presets for the first two rather than branching on a "type". An organization's Settings → Model policy now picks from `listSelectableModels()` (gateway-admin-curated, validated server-side against the real list on submit) instead of typing a raw `kilo/...` string.

The resolved credential flows through the existing pilot↔pilot-ai contract rather than a new channel: `buildRuntimeRequest` resolves a `gw:<gatewayId>:<modelId>` selection to `{modelId: "openai/<modelId>", apiKey, baseUrl}`, `runtime-contract.ts`/`pilot-ai-client.ts` carry `gatewayApiKey`/`gatewayBaseUrl` as two new headers, and pilot-ai's `agent-factory.ts` passes them straight to Mastra's model router as its documented per-call `{id, url, apiKey}` override — no new pilot-ai-side provider registration needed. A request with no gateway credential still resolves `modelId` from pilot-ai's own environment as before, so this is additive, not a breaking cutover. Both repos' `modelId` regexes were relaxed from Kilo-only to any `<provider>/<model>` id, and pilot-ai's contract test for this was updated to assert the new, intentionally-broader acceptance instead of the old rejection.

Two dead hardcoded gates were also removed while touching this path: `loadRuntimeAgent`/`startConversation` used to refuse any agent whose stored `workers.modelId` wasn't exactly the one hardcoded default, even though that column was never actually read for the live model choice (the organization's model policy always was) — a latent trap that could make a conversation permanently "unavailable" for no visible reason.

Verified locally: `pnpm check` and `pnpm test:server`/`pnpm test:db` clean on both repos (pilot-ai: 70/70 vitest, including new contract tests for the relaxed model id and the gateway credential fields); exercised the `model-gateways`/`platform-admin` repository functions directly against the local dev database (create/disable/delete a gateway, resolve its credential back out, list admins) — not through the actual `/admin` browser UI, which is still unverified by hand. Also not yet done: an actual end-to-end reply through a real gateway credential (the dev environment's existing Kilo key was never migrated into a `model_gateways` row as part of this change — an admin needs to add one), and production migration.

## Work standing instructions, and a correction to the "Pilot Work foundation" entry above (2026-09-18)

The "Pilot Work foundation" entry above (2026-09-13) describes an owner-scoped task queue at `/workspace/work` with `ready`/`completed`/`cancelled` states. That whole substrate was superseded one day later by [ADR-0016](decisions/0016-three-agent-kinds.md) (implemented 2026-09-14, see "Current state (2026-09-15)" above) and no longer exists: migration 0028 dropped its tables outright. Today "Work" is simply `workers.baseAgentId = 'work'` on an ordinary conversation, riding the exact same `src/conversations/` + `src/executions/` machinery as Chat and Code — no queue, no status column, no dispatch concept, nothing bolted on. Anyone reading that entry for current behavior should read this one instead.

Given that, and per this repo's memory that the old approvals/tasks substrate must never be revived, and that a durable autonomous-execution runtime needs its own persistence/recovery/cancellation/budget contract too large for one slice, this pass implemented the smallest concrete step from `pilot-work-should-be-groovy-orbit.md`'s Phase F.3 that needed no product decision: per-user **standing instructions for Work**, matching Cowork's "Settings > Cowork" template. `user_preferences.work_instructions` (migration `0047_work_instructions.sql`, additive/nullable text) is set once in Settings → Work and applied automatically to every Work conversation the user starts or continues, via a new optional `standingInstructions` parameter on `buildAgentInstructions` (`src/agents/agent-instructions.ts`) — built server-side as always, never accepted from the browser. `startConversation` and `loadRuntimeAgent` (`src/conversations/`) fetch it only when `agent.baseAgentId === "work"`; Chat and Code never see it, and a request for one user's instructions never returns another's.

Verified locally: `pnpm check` (lint/typecheck/format/knip clean), `pnpm build`, `pnpm test` (24/24 Playwright), `pnpm test:server` (79/79), `pnpm test:db` (14/14, including two new integration tests asserting the kind gate and per-user isolation), `pnpm audit --audit-level high` (clean). Not verified: the actual `/settings` Work tab through an authenticated browser session (WorkOS sign-in not exercised in this pass, consistent with several earlier slices above).

Still open from the plan, in priority order, for a future session:

- **Phase A.1 (durable dispatch loop)** — the real gap. Today every kind, including Work, is a single synchronous request/response turn; there is no multi-step autonomous execution, resumable run, or budget to surface. This needs its own persistence/recovery/cancellation/budget contract, explicitly out of scope for one slice, and is the prerequisite for almost everything else the plan describes (steering, budgets, scheduling).
- **Phase A.3 (session approval mode: manual/auto/skip)** — skipped, not merely deferred: it targets `authorize()`/`policy-classes.ts`, which no longer exist in this codebase (removed with the old approvals/tasks substrate). Per `AGENTS.md`'s Product model, an enabled tool "just runs — no approval or suspension step," and the connector engine's own `isMutating`/`confirm` gate is already the whole approval mechanism (ADR-0023). Reintroducing a mode that suspends read/local actions pending approval would contradict that settled decision, not extend it — this needs a product decision (whether Work should behave differently here from Chat/Code at all) before any code.
- **Phase B.2 (deliverable generation)** — documents/spreadsheets/decks as real files, reusing `src/files/` and the existing `@react-pdf/renderer` export pipeline. Buildable without the dispatch loop (a tool can produce a file within one turn), but needs a matching Mastra tool in `pilot-ai`, so it's a cross-repo slice, not attempted here.
- **Phase C/D (browser automation, connector write actions)** — the connector engine already supports `isMutating`/`confirm` end to end (ADR-0023), but per `docs/progress.md`'s "Current state" section, no connector has a real write action defined yet; the first one would be a good next slice, and does not require a product decision, only picking a connector and an action.
- **Phase E/F** — scheduling and shared projects need the dispatch loop first; enterprise admin controls and citations are polish deferred behind all of the above.
