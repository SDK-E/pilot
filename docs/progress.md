...
The proposed [Mastra conversation runtime contract](decisions/0004-mastra-conversation-runtime-contract.md) preserves this boundary for the next slice. It defines the organization-and-worker memory resource, Conversation thread, Neon-backed storage requirement, server-to-server authorization requirement and no-tools policy. Kilo Gateway model `kilo/kilo-auto/free` is the selected development model. The contract is still not an implementation: Neon-backed runtime storage and authenticated Pilot-to-runtime transport are required before a generation can be exposed.
